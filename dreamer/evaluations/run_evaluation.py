...
def get_checkpoint_regex(checkpoint_dir, track, agent, task, obs_type):
    print("DEBUG **************************")
    print("DEBUG variables: ", checkpoint_dir, track, agent, task, obs_type)
    print("DEBUG joined checkpoint path: ", str(checkpoint_dir / f'{track}_{agent}_{task}_{obs_type}*'))
    print("DEBUG **************************")
    #return str(checkpoint_dir / f'{track}_{agent}_{task}_{obs_type}_[0-9]*')     #original
    #checkpoint_folder = glob.glob(str(checkpoint_dir / f'{track}_{agent}_{task}_{obs_type}*'))
    #return str(checkpoint_dir / f'{track}_{agent}_{task}_{obs_type}*')
    #print(type(checkpoint_folder[0]))
    return "logs/experiments/circle-cw_dreamer_max_progress_lidar_tanhnormal_Ar4_Bl50_H15_1226_1670954545.6184576/checkpoints/*.pkl"


def glob_checkpoints(checkpoint_dir, track, agent, task, obs_type="*"):
    regex = get_checkpoint_regex(checkpoint_dir, track, agent, task, obs_type)
...
def main(args):
    ...
    if args.agent == "ftg":
        ...
        #print("DEBUG: in else block")
        # find all checkpoints in `checkpoint_dir` for the given agent` and training track
        checkpoints = glob_checkpoints(args.checkpoint_dir, args.trained_on, args.agent, args.task, args.obs_type)
        print(checkpoints[3])
        #agent = RacingAgent(args.agent, checkpoints[0], obs_type=args.obs_type, action_dist=args.action_dist)      #original
        chkpt = "logs/experiments/circle-cw_dreamer_max_progress_lidar_tanhnormal_Ar4_Bl50_H15_1226_1670954545.6184576/checkpoints/96452.pkl"
        agent = RacingAgent(args.agent, chkpt, obs_type=args.obs_type, action_dist=args.action_dist)
...
def parse():
    tracks = ['austria', 'columbia', 'barcelona', 'gbr', 'treitlstrasse-v2','circle-cw','SILVERSTONE','SOCHI','Oschersleben']
    agents = ["dreamer", "d4pg", "mpo", "ppo", "sac", "ftg"]
    parser = argparse.ArgumentParser()
    parser.add_argument('--agent', type=str, choices=agents, required=True)
    parser.add_argument('--task', type=str, required=False, default="max_progress")
