...
def parse_file(dir, file, file_parsers):
    ...
    #print("***[DEBUG]*** dir, file, file_parsers: ", dir, file, file_parsers)
    #print("***[DEBUG in parse_file()]*** filepath: ", filepath)
    #file_parser = file_parsers[1]
    #train_track, method, seed = file_parser(filepath)
    #print("***[DEBUG in parse_file()]*** train_track, method, seed: ", train_track, method, seed)
    for file_parser in file_parsers:
        try:
            #print("***[DEBUG]*** file_parser: ", file_parser)
            train_track, method, seed = file_parser(filepath)
...
def load_runs(args, file_parsers, tag, eval_mode=False):
  ...
  for dir in args.indir:
    ...
    for file in dir.glob('**/events*'):
        ...
        #print("***[DEBUG]*** train_track, method, seed: ", train_track, method, seed)
        try:
            #print("***[DEBUG in load_runs()]*** dir, file, file_parsers: ", dir, file, file_parsers)
            train_track, method, seed = parse_file(dir, file, file_parsers)
            #print("***[DEBUG in load_runs()]*** train_track, method, seed: ", train_track, method, seed)
            if not check_track(train_track, args.tracks) or not check_method(method, args.methods):
                print("***[DEBUG in load_runs()]*** in if check_track() check_method()")
                continue
            event_acc = EventAccumulator(str(file), size_guidance={'scalars': 1000,
                                                                   'tensors': 1000})  # max number of items to keep
            event_acc.Reload()
            print("***[DEBUG in load_runs()]*** in event_acc.Reload() done!")
        except Warning as w:
            print("***[DEBUG in load_runs()]*** in Warning")
            warnings.warn(w)
            continue
        except Exception as err:
            print("***[DEBUG in load_runs()]*** in Exception")
            print(f'Error {file}: {err}')
            continue
        if eval_mode:
            print("***[DEBUG in load_runs()]*** eval_mode: ", eval_mode)
