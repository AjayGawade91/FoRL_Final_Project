...
def plot_filled_curve(args, runs, axes, aggregator):
    ...
    for i, (track, ax) in enumerate(zip(tracks, axes)):
        ...
        #ax.set_xticks(range(0, 8000001, 2000000))      #original
        ax.set_xticks(range(0, 125001,  25000))          #set x-axis range
...
def main(args):
    ...
    print("***[DEBUG in main()]*** runs:", runs)
    tracks = sorted(set([r.train_track for r in runs]))
    print("***[DEBUG in main()]*** tracks:", tracks)
