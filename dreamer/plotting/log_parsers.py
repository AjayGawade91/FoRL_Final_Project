...
class ModelFreeParser(Parser):
    #print("***[DEBUG in ModelFreeParser]*** before __call__(): ")
    def __call__(self, logdir):
        #print("***[DEBUG in ModelFreeParser]*** in __call__(): ")
        splitted = logdir.split('_')
        #print("***[DEBUG in ModelFreeParser]*** splitted: ", splitted)
...
class DreamerParser(Parser):
    ...
    def __call__(self, logdir):
        #print("***[DEBUG in DreamerParser]*** in __call__(): ")
        #print("***[DEBUG in DreamerParser]*** logdir: ", logdir)
        splitted = logdir.split('_')
        #print("***[DEBUG in DreamerParser]*** splitted: ", splitted)
        if len(splitted) == 10:  # assume logdir: track_dreamer_max_progress_ArK_BlL_HH_seed_timestamp
            track, algo, _, _, _, action_repeat, batch_len, horizon, seed, _ = splitted
            base_algo = algo
        elif len(splitted) == 11:  # assume logdir: track_dreamer_max_progress_method_ArK_BlL_HH_seed_timestamp
            track, algo, _, _, method, _, action_repeat, batch_len, horizon, seed, _ = splitted
